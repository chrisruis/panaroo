...
def max_clique(tG):
    # greedy, very approximate max clique method
    # assumes the maximum clique will include the node of largest size

    # get largest node
    nodes = sorted(tG.nodes, key=lambda x: tG.nodes[x]['size'], reverse=True)
    ...
    for n in nodes:
        should_add = True
        for n_clique in max_clique:
            if not tG.has_edge(n, n_clique):
                should_add = False
                break
        if should_add:
            max_clique.append(n)
