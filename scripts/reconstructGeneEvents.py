...
if __name__ == "__main__":
    ...
    parser.add_argument("-g", help = "final_graph.gml from Panaroo")
    parser.add_argument("-r", help = "combined_ancestral_states.tab from PastML")
    ...
    geneNodeOutFile = open(args.o + ".branch.gene.gains.losses.txt","w")
    geneNodeOutFile.write("Branch\tGenes.gained\tGenes.lost\n")
    labelledTreeOutFile = open(args.o + ".tree.with.node.labels.nex","w")
    eventNodeOutFile = open(args.o + ".branch.event.gains.losses.txt","w")
    eventNodeOutFile.write("Branch\tEvents.gained\tEvents.lost\n")
    eventNodeSummaryOutFile = open(args.o + ".branch.event.number.gains.losses.txt","w")
    eventNodeSummaryOutFile.write("Branch\tEvents.gained\tEvents.lost\n")
    eventSummaryOutFile = open(args.o + ".event.summary.txt","w")
    eventSummaryOutFile.write("Event\tNumber.of.genes\tGenes\tBranch\tGain.or.loss\n")

    geneGraph = nx.read_gml(args.g) #Import the gene graph

    reconstruction = open(args.r).readlines() #Import the gene reconstructions
    ...
    for node in reconstruction: #Iterate through the nodes and remove the second line for the internal nodes
        if node.strip().split("\t")[0] not in processedNodes:
            geneReconstruction.append(node)
            processedNodes.append(node.strip().split("\t")[0])
            if node.strip().split("\t")[0] != "node": #Append the gene presence absence information for the node
                geneDictionary[node.strip().split("\t")[0]] = "".join(node.strip().split("\t")[1:])
    
    geneNames = reconstruction[0].strip().split("\t")[1:] #The name of each gene

    nodeGenes = [] #Will be filled with each node and the genes that are gained and lost along the corresponding branch
    
    tree = Phylo.read(args.t,"newick") #Import the tree
    ...
    for clade in labelledTree.find_clades(): #Iterate through the clades
        if len(tree.get_path(clade)) != 0: #Do not analyse the root
            ...
            if upstreamNode == "root": #Check if the upstream node is the root
                differentGenes = [i + j for i,j in zip(geneDictionary["root"],geneDictionary[clade.nodeName])] #1 or 0 for each gene if the gene is present or absent
    ...
    for branch in nodeGenes: #Iterate through the branches
        ...
        for node in geneGraph: #Iterate through the nodes in the gene graph
            if geneGraph.node[node]["name"] in branch[1]: #Check if the node is gained along the branch
                genes = [[geneGraph.node[node]["name"]],[]] #Append the gene name and an empty list, the empty list will be filled with the neighbours
                for geneNeighbour in geneGraph.neighbors(node):
                    genes[1].append(geneGraph.node[geneNeighbour]["name"])
                gainNeighbours.append(genes)
                gainEvents = joinGenes(gainNeighbours) #Assign to the gene gain events
            elif geneGraph.node[node]["name"] in branch[2]: #Check if the node is lost along the branch
                genes = [[geneGraph.node[node]["name"]],[]]
                for geneNeighbour in geneGraph.neighbors(node):
                    genes[1].append(geneGraph.node[geneNeighbour]["name"])
